...
// Test virtual functions and inheritance in TA
...
int main()
{
    ...
    std::cout << "========================================" << std::endl;
    std::cout << "Testing Virtual Functions & Inheritance" << std::endl;
    ...
    res = TEEC_InvokeCommand(&sess, TA_EEVM_CMD_TEST_VIRTUAL, &op, &err_origin);
    ...
    if (res == TEEC_SUCCESS) {
        std::cout << "\n✓ Virtual functions test PASSED!" << std::endl;
    } else {
        std::cerr << "\n✗ Virtual functions test FAILED: 0x" << std::hex << res 
                  << " origin 0x" << err_origin << std::endl;
    }
