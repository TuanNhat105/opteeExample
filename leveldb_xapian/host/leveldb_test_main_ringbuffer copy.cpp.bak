...
int main()
{
    ...
    std::memset(&op, 0, sizeof(op));
    // Use TEEC_MEMREF_WHOLE for allocated shared memory
    // This is the correct way to pass allocated shared memory to TA
    ...
    // For TEEC_MEMREF_WHOLE, we only need to set parent
    op.params[0].memref.parent = &shm;
    // offset and size are ignored for WHOLE, but set them anyway for safety
    ...
    std::cout << "Invoking INIT_LEVELDB with shared memory size: " << shm.size << std::endl;
    ...
    std::cout << "[DEBUG] Calling TEEC_InvokeCommand, waiting for TA response..." << std::endl;
    std::cout << "[DEBUG] Shared memory validation:" << std::endl;
    std::cout << "  - shm.buffer: " << std::hex << shm.buffer << std::dec << std::endl;
    std::cout << "  - shm.size: " << shm.size << std::endl;
    std::cout << "  - op.params[0].memref.parent: " << std::hex << op.params[0].memref.parent << std::dec << std::endl;
    std::cout << "  - op.params[0].memref.offset: " << op.params[0].memref.offset << std::endl;
    std::cout << "  - op.params[0].memref.size: " << op.params[0].memref.size << std::endl;
    std::cout.flush();
    
    // Add timeout mechanism - if command takes too long, it's hanging
    std::cout << "[DEBUG] Invoking command (will timeout if takes > 30 seconds)..." << std::endl;
    std::cout.flush();
    ...
    if (log_buffer[0] != '\0') {
        ...
        std::cout << "--- End TA Logs ---\n" << std::endl;
    } else {
        std::cout << "[DEBUG] No logs received from TA (log_buffer is empty)" << std::endl;
    }
    
    if (res != TEEC_SUCCESS) {
        std::cerr << "❌ LevelDB initialization failed: 0x" << std::hex << res << std::dec << std::endl;
        std::cerr << "Error origin: 0x" << std::hex << err_origin << std::dec << std::endl;
        consumer.Stop();
    ...
    if (res != TEEC_SUCCESS) {
        ...
        if (status == 0) {
            ...
            std::cout << "[DEBUG] Retrieved value size: " << value_size << " bytes" << std::endl;
            if (value_size > 0) {
