...
namespace eevm
{
  ...
  uint64_t Stack::pop64()
  {
    ...
    {
      // Use intx::hex directly to avoid fmt::format dependency (OP-TEE compatible)
      std::string hex_val = "0x" + intx::hex(val);
      throw Exception(
        ET::outOfBounds,
        "Value on stack (" + hex_val + ") is larger than 2^64");
    }
  ...
  std::ostream& operator<<(std::ostream& os, const Stack& s)
  {
    ...
    {
      // Use intx::hex directly instead of to_hex_string to avoid fmt::format dependency
      // This maintains the same output format without requiring fmt library (OP-TEE compatible)
      os << " " << i++ << ": 0x" << intx::hex(elem) << std::endl;
    }
    return os;
  }
} // namespace eevm
