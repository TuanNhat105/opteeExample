...
#include "eEVM/trace.h"  // Include here instead of processor.h - OP-TEE compatible
#include "eEVM/util.h"
    ...
    void dispatch()
    {
      ...
      switch (op)
      {
        ...
        case Opcode::STOP:
          ...
          break;
        default:
          // OP-TEE compatible: Replace fmt::format with standard C++ streams
          stringstream err;
          err << "Unknown/unsupported Opcode: 0x" 
              << std::hex << std::setw(2) << std::setfill('0') 
              << int{get_op()} << std::dec << endl;
          err << " in contract " 
              << to_checksum_address(ctxt->as.acc.get_address()) << endl;
          err << " called by " 
              << to_checksum_address(ctxt->caller) << endl;
          err << " at position " << ctxt->get_pc() 
              << ", call-depth " << get_call_depth() << endl;
