  ...
  namespace rlp
  {
    ...
    namespace decode_details
    {
      ...
      struct from_bytes<size_t>
      {
        size_t operator()(const uint8_t*& data, size_t& size)
        {
          if (size > 8)
          {
            // OP-TEE compatible: Use stringstream instead of fmt::format
            std::ostringstream oss;
            oss << "Trying to decode number: " << size << " is too many bytes for uint64_t";
            throw decode_error(oss.str());
          }
      ...
      struct from_bytes<std::array<uint8_t, N>>
      {
        std::array<uint8_t, N> operator()(const uint8_t*& data, size_t& size)
        {
          if (size != N)
          {
            // OP-TEE compatible
            std::ostringstream oss;
            oss << "Trying to decode " << N << " byte array, but given " << size << " bytes to decode";
            throw decode_error(oss.str());
          }
        ...
        const uint8_t*& data, size_t& size)
      {
        ...
        if (length <= 0xbf)
        {
          ...
          if (size < length_of_length)
          {
            // OP-TEE compatible
            std::ostringstream oss;
            oss << "Length of next element should be encoded in " << length_of_length << " bytes, but only " << size << " remain";
            throw decode_error(oss.str());
          }
        ...
        if (size < length_of_length)
        {
          // OP-TEE compatible
          std::ostringstream oss;
          oss << "Length of next list should be encoded in " << length_of_length << " bytes, but only " << size << " remain";
          throw decode_error(oss.str());
        }
      ...
      std::tuple<Ts...> decode_impl(const uint8_t*& data, size_t& size)
      {
        ...
        if constexpr (sizeof...(Ts) == 0)
        {
          if (contained_length != 0)
          {
            // OP-TEE compatible
            std::ostringstream oss;
            oss << "Expected empty list, but data contains " << contained_length << " remaining bytes";
            throw decode_error(oss.str());
          }
    ...
    std::tuple<Ts...> decode(const uint8_t*& data, size_t& size)
    {
      ...
      if (size != 0)
      {
        // OP-TEE compatible
        std::ostringstream oss;
        oss << "Expected to decode entire input, but " << size << " bytes remain";
        throw decode_error(oss.str());
      }
