...
// OP-TEE compatible: Remove fmt dependency, use standard C++ only
#include <iomanip>
#include <limits>
...
namespace eevm
{
  ...
  // OP-TEE compatible: Replace fmt::format with standard C++ stringstream
  ...
  std::string to_hex_string(Iterator begin, Iterator end)
  {
    ...
    {
      oss << std::setw(2) << static_cast<int>(static_cast<uint8_t>(*it));
    }
    return oss.str();
  }
    ...
    const uint256_t& v, size_t min_hex_chars = 64)
  {
    std::ostringstream oss;
    oss << intx::hex(v);
    auto hex_str = oss.str();
    if (hex_str.size() < min_hex_chars)
    {
      hex_str = std::string(min_hex_chars - hex_str.size(), '0') + hex_str;
    }
    return "0x" + hex_str;
  }
