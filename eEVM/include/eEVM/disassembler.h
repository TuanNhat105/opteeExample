...
namespace eevm
{
  ...
  {
    // OP-TEE compatible: Use stringstream instead of fmt
    os << std::setw(5) << std::right << i.pc << ": ";
    os << i.op.mnemonic;
    
    if (i.op.has_immediate())
    {
      os << " " << to_lower_hex_string(i.get_immediate());
    }
    
    os << " [" << std::hex << std::setfill('0') << std::setw(2) 
       << (int)i.op.opcode << std::dec;
    
    if (i.raw_imm.size() > 0)
    {
      os << " ";
      for (size_t idx = 0; idx < i.raw_imm.size(); ++idx)
      {
        if (idx > 0) os << " ";
        os << std::hex << std::setfill('0') << std::setw(2) 
           << (int)i.raw_imm[idx] << std::dec;
      }
    }
    ...
    static Disassembly dis(const std::vector<uint8_t>& prog)
    {
      ...
      for (auto it = prog.cbegin(); it != prog.cend();)
      {
        ...
        if (bytes_left < op.immediate_bytes)
        {
          ...
          oss << "Immediate exceeds instruction stream (op " << op.mnemonic 
              << " at instruction " << (size_t)(it - prog.cbegin())
              << " wants " << (int)op.immediate_bytes 
              << " bytes, only " << bytes_left << " remain)";
